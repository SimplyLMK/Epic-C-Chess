...
namespace custom_projD
{
    public class Board
    {
        ...
        // check if the coordinate is valid to prevent the piece to move out of the array
        ...
        // draw every instance of the pieces to the board array
        public void Draw()
        {
            for (int row = 0; row < 8; row++)
            {
        ...
        // Handling piece selection and deselection
        public bool Select_Piece(int mouseX, int mouseY, int squareSize)
        {
            ...
            Piece clicked_Piece = Coordinate(Rowed, Columned); // get the valid coordinate

            if (_selected != null)
            {
                if (_selected == clicked_Piece && clicked_Piece.Is_Selected(mouseX, mouseY, squareSize))
                {
                    Deselect_Piece(); // call the deselect method
                }
                else
                {
                    // 
